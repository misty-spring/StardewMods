...
{
    private static string AfterEatingKey { get; set; } = $"{ModEntry.Id}/AfterEatingAnimation";
    private static string CustomEatingKey { get; set; } = $"{ModEntry.Id}/EatingAnimation";
#if DEBUG
    ...
    internal static bool Pre_eatObject(Farmer __instance, Object o, bool overrideFullness = false)
    {
        ...
        if (!ModEntry.Data.TryGetValue(o.QualifiedItemId, out var data))
        {
            ...
            if (!objectData.CustomFields.TryGetValue(CustomEatingKey, out var customField))
                return true;
    ...
    private static void AnimateFromMod(Farmer who, FarmerAnimation data)
    {
        ...
        try
        {
            if (AnimateFromObject(who, data.Animation))
            {
                return;
            }
            else
    ...
    private static void CheckItemAnimation(FarmerAnimation animation, Farmer who)
    {
        ...
        var animatedSprite = new TemporaryAnimatedSprite(
            ...
            animationInterval: animation.Food.Length,
        ...
        {
            var delay = animation.Food.Length * animation.Food.Frames + animation.Food.Delay;

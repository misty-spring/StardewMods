...
{
    private static Dictionary<string, List<ExtraTrade>> _extraSaleItems;
    //private static bool ShouldDraw => ShopPatches.InMenu;
#if DEBUG
    ...
    {
        ModEntry.ExtraTrades.TryGetValue(__instance.ShopId, out _extraSaleItems);
        Log($"shopId {__instance.ShopId}, ExtraTrades {ModEntry.ExtraTrades.Count}, items in id entry?: {_extraSaleItems?.Count}");
    }
    ...
    { 
        ...
        if(_extraSaleItems is not { Count: > 0 })
            return;
        
        //Log($"Hovered item: {hoveredItem.DisplayName}");
        
        if(!_extraSaleItems.TryGetValue(hoveredItem.QualifiedItemId, out var data))
            return;
        ...
        if (hoveredItem != null)
        {
            ...
            switch (hoveredItem)
            {
                ...
                case StardewValley.Object @object when @object.Edibility != -300:
                {
                    if (healAmountToDisplay != -1)
            ...
            if (buffIconsToDisplay != null)
            {
              ...
              { 
                  if (!buffIconsToDisplay[index2].Equals("0") && index2 <= 11) 
                      num1 = (int) Math.Max(num1, font.MeasureString(Game1.content.LoadString("Strings\\UI:ItemHover_Buff" + index2.ToString(), sub1)).X + horizontalBuffer);
              }
            }
        }
        ...
        if (healAmountToDisplay != -1)
        {
            var num8 = (hoveredItem as StardewValley.Object).staminaRecoveredOnConsumption();
            if (num8 >= 0)
            { 
                var num9 = (hoveredItem as StardewValley.Object).healthRecoveredOnConsumption();
                ...
                Utility.drawTextWithShadow(b, Game1.content.LoadString("Strings\\UI:ItemHover_Energy", (num8 > 0 ? "+" : "") + num8.ToString()), font, new Vector2(x + 16 + 34 + 4, y2 + 16), Game1.textColor); 
                y2 += 34; 
                if (num9 > 0)
                {
                    ...
                    // ReSharper disable once ConditionIsAlwaysTrueOrFalse
                    Utility.drawTextWithShadow(b, Game1.content.LoadString("Strings\\UI:ItemHover_Health", (num9 > 0 ? "+" : "") + num9.ToString()), font, new Vector2(x + 16 + 34 + 4, y2 + 16), Game1.textColor);
                    y2 += 34;
                }
            }
            else if (num8 != -300)
        ...
        if (buffIconsToDisplay != null)
        {
            for (var index3 = 0; index3 < buffIconsToDisplay.Length; ++index3)
            {
                ...
                if (index3 <= 11)
                {
                    str = Game1.content.LoadString("Strings\\UI:ItemHover_Buff" + index3.ToString(), str);
                }
    ...
    internal static bool Pre_receiveLeftClick(ShopMenu __instance, int x, int y, bool playSound = true)
    {
        ...
        for (var index1 = 0; index1 < __instance.forSaleButtons.Count; ++index1)
        {
            ...
            if (!_extraSaleItems.ContainsKey(thisItem.QualifiedItemId))
                return true;
    ...
    private static bool CanPurchase(ISalable item, int stockToBuy)
    {
        ...
        #if DEBUG
        Log($"item: {item.DisplayName}, stockToBuy {stockToBuy}, in _extraSaleItems {_extraSaleItems.Count}");
        #endif
        
        if(_extraSaleItems is not { Count: > 0 })
            return false;
        
        if(!_extraSaleItems.TryGetValue(item.QualifiedItemId, out var data))
            return false;
        ...
        ref bool __result)
    {
        ...
        foreach (var extra in data)
        {
            if (HasMatch(Game1.player, extra, false, stockToBuy))
                continue;
        ...
        ref bool __result)
    {
        ...
        foreach (var extra in data)
        {
            if (HasMatch(Game1.player, extra, true, stockToBuy))
                continue;
            
            Log($"Error when removing trade items: not enough in inventory. Item {extra.QualifiedItemId}, minimum quality {extra.ActualQuality}, count {extra.Count} ({extra.Count * stockToBuy} total)", LogLevel.Error);
            
            __instance.emergencyShutDown();
        }
    }*/
    ...
    private static bool HasMatch(Farmer farmer, ExtraTrade c, int bought = 1)
    {
        ...
        foreach (var item in rightQuality)
        {
            if (item.Stack >= withStock)
            {
                item.ConsumeStack(withStock);
                return true;
            }

            withStock -= item.Stack;
            item.ConsumeStack(item.Stack);
        }

        if (withStock > 0)
            throw new NotSupportedException("Not enough stock to reduce for trade.");

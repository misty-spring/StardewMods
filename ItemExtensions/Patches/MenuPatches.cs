...
public static class MenuPatches
{
    ...
    {
        Log($"Applying Harmony patch \"{nameof(MenuPatches)}\": postfixing SDV method \"InventoryMenu.rightClick(int, int, Item, bool, bool)\".");
        harmony.Patch(
            original: AccessTools.Method(typeof(InventoryMenu), nameof(InventoryMenu.rightClick)),
            postfix: new HarmonyMethod(typeof(MenuPatches), nameof(Post_rightClick))
        );
    }
    ...
    internal static void Post_rightClick(InventoryMenu __instance, int x, int y, ref Item toAddTo, ref Item __result, bool playSound = true, bool onlyCheckToolAttachments = false)
    {
        ...
        if (ModEntry.ItemActions == null || ModEntry.ItemActions?.Count == 0)
            return;


        // ReSharper disable once PossibleNullReferenceException
        if (!ModEntry.ItemActions.TryGetValue(heldItem.QualifiedItemId, out var options))
            return;
        
        Log("Found conversion data for item.", LogLevel.Debug);
        ...
        foreach (var data in options)
        {
            ...
            {
                Log($"Conditions for {data.TargetID} don't match.", LogLevel.Debug);
                break;
            }
            ...
            {
                Log($"Minimum to remove from {data.TargetID} isn't avaiable.", LogLevel.Debug);
                break;
            }
            ...
            if(heldItem.Stack < data.RemoveAmount * affectedItem.Stack && Game1.player.freeSpotsInInventory() == 0)
            {
                ...
                Log("No spaces avaiable in inventory. Can't partially convert stack.", LogLevel.Debug);
                break;
            }
            ...
            if (!string.IsNullOrWhiteSpace(data.ReplaceBy))
            {
                ...
                else
                {
                    ...
                    Log($"Removing {data.RemoveAmount} for each converted item.", LogLevel.Debug);
